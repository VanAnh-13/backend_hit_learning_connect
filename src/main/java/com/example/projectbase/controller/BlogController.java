...
public class BlogController {
    ...
    @Tag(name = "blog-ADMIN-controller")
    @PreAuthorize("hasAnyRole('ADMIN','LEADER')")
    @PutMapping("/{blogId}/approval")
    public ResponseEntity<String> approveOrRejectBlog(@PathVariable Long blogId,
                                                      @RequestBody BlogApprovalRequestDto requestDto) {
        blogService.approveOrRejectBlog(blogId, requestDto);
        return ResponseEntity.ok(ResponseMessage.Blog.UPDATE_STATUS);
    }

    @Tag(name = "blog-ADMIN-controller")
    @PreAuthorize("hasAnyRole('ADMIN','LEADER')")
    @GetMapping("/pending")
    public ResponseEntity<Page<BlogResponse>> getPendingBlogs(Pageable pageable) {
    ...
    public ResponseEntity<Page<BlogStatiticReponseDto>> getBlogStatistics(
            @RequestParam(required = false) String fromDate,
            @RequestParam(required = false) String toDate,
            @RequestParam(required = false) String author,
            @RequestParam(required = false) String tag,
            Pageable pageable) {
        return ResponseEntity.ok(blogService.getBlogStatistics(fromDate, toDate, author, tag, pageable));
    }
}
